use crate::project_02::{Add16, Inc16, Zero16, flatten, flatten_for_simulation};
...
use simulator::{Chip as _, Reflect};

fn synth<C: Reflect + Clone + Into<crate::project_02::Project02Component>>(
    chip: C,
) -> ChipWiring<N16> {
...
fn add16_wiring() {
    ...
    assert_eq!(
        ops.ripple_adders, 1,
        "16 adders should coalesce into 1 ripple adder"
    );
...
fn zero16_wiring() {
    ...
    let ops = wiring.op_counts();
    assert_eq!(
        ops.parallel_nands, 1,
        "Not16 should coalesce into 1 parallel nand"
    );
    assert_eq!(
        ops.many_way_ands, 1,
        "and-tree + final not should coalesce into 1 many-way and"
    );
